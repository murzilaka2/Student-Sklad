...
namespace Sklad.Windows
{
    public partial class ViewProduct : Form
    {
        ...
        public ViewProduct(Main main, Models.Product product)
        {
            InitializeComponent();
            this.main = main;
            main.Hide();
        ...
        {
            foreach (Models.Storage item in StorageComboBox.Items)
            {
                if (item.Id == product.Storage.Id)
                {
        ...
        private void HideProductViewButton_Click(object sender, EventArgs e)
        {
            this.WindowState = FormWindowState.Minimized;
        }
        private void MinimizeProductViewButton_Click(object sender, EventArgs e)
        {
        ...
        private void CloseProductViewButton_Click(object sender, EventArgs e)
        {
            Application.Exit();
        }
        ...
        private void Update()
        {
            ...
            if (!string.IsNullOrWhiteSpace(NameTextBox.Text) && !string.IsNullOrWhiteSpace(DescriptionTextBox.Text) && !string.IsNullOrWhiteSpace(TypeTextBox.Text))
            {
                ...
                using (Models.ApplicationContext db = new Models.ApplicationContext())
                {
                    var editProduct = db.Products.FirstOrDefault(e => e.Id == product.Id);
                    if (editProduct is not null)
                    {
                        if (editProduct.StorageId != storage.Id)
                        {
                            StorageMove(storage.Id);
                        }
                        editProduct.StorageId = storage.Id;
                        editProduct.Name = NameTextBox.Text;
                        editProduct.Description = DescriptionTextBox.Text;
                        editProduct.Type = TypeTextBox.Text;
                        editProduct.Count = (int)CountNumeric.Value;
                        editProduct.Price = PriceNumeric.Value;
                        editProduct.Date = DatePicker.Value;
                    }
                    db.SaveChanges();
                }
                using (Models.ApplicationContext db = new Models.ApplicationContext())
                {
                    db.Sheets.Add(new Models.Sheet() { ActionType = Models.Action.Editing, ProductName = product.Name, StorageName = db.Storages.Where(e => e.Id == product.StorageId).FirstOrDefault()!.Name });
                    db.SaveChanges();
                }
        ...
        void StorageMove(int newStorageId)
        {
            using (Models.ApplicationContext db = new Models.ApplicationContext())
            {
                db.Sheets.Add(new Models.Sheet() { ActionType = Models.Action.Movement, ProductName = product.Name, StorageName = db.Storages.Where(e => e.Id == newStorageId).FirstOrDefault()!.Name,
                    OldStorageName = db.Storages.Where(e => e.Id == product.StorageId).FirstOrDefault()!.Name
                });
                db.SaveChanges();
            }
        }
        ...
        private void ViewProduct_Resize(object sender, EventArgs e)
        {
            AdjustForm();
        }
        ...
        private void RemoveButton_Click(object sender, EventArgs e)
        {
            using (Models.ApplicationContext db = new Models.ApplicationContext())
            {
                var removeProduct = db.Products.FirstOrDefault(e => e.Id == product.Id);
                db.Products.Remove(removeProduct!);
                db.SaveChanges();
            }
            using (Models.ApplicationContext db = new Models.ApplicationContext())
            {
                db.Sheets.Add(new Models.Sheet() { ActionType = Models.Action.Removal, ProductName = product.Name, StorageName = db.Storages.Where(e => e.Id == product.StorageId).FirstOrDefault()!.Name });
                db.SaveChanges();
            }
            this.DialogResult = DialogResult.OK;
            main.Top = this.Top;
        ...
        private void Enable_Items()
        {
            ...
            StorageComboBox.Enabled = true;
            NameTextBox.Enabled = true;
            DescriptionTextBox.Enabled = true;
            TypeTextBox.Enabled = true;
            DatePicker.Enabled = true;
            CountNumeric.Enabled = true;
            PriceNumeric.Enabled = true;

            NameTextBox.BorderStyle = BorderStyle.Fixed3D;
            DescriptionTextBox.BorderStyle = BorderStyle.Fixed3D;
            TypeTextBox.BorderStyle = BorderStyle.Fixed3D;
            CountNumeric.BorderStyle = BorderStyle.Fixed3D;
            PriceNumeric.BorderStyle = BorderStyle.Fixed3D;

            StorageComboBox.BackColor = Color.White;
            NameTextBox.BackColor = Color.White;
            DescriptionTextBox.BackColor = Color.White;
            TypeTextBox.BackColor = Color.White;
            DatePicker.BackColor = Color.White;
            CountNumeric.BackColor = Color.White;
            PriceNumeric.BackColor = Color.White;
        }
